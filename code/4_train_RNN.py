...
from tensorflow.keras.optimizers import legacy as opt_legacy
model.compile(optimizer=opt_legacy.Adam(), loss='mean_squared_error')
...
for name, group in grouped:
    ...
    for i in range(len(group) - 10):
        sequences.append(group.iloc[i:i+10, :-1].values)  # 最後の列がラベルでないと仮定
...
# モデルの保存
model.save('./model/rnn_model.keras', save_format='tf')
